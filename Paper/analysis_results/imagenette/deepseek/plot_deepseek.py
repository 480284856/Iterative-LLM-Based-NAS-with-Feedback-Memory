"""
Plot three accuracy curves for DeepSeek-Coder-6.7B-Instruct (2000 iterations):
...
RESULTS_LOG = HERE.parent.parent.parent / "experimental_result/imagenette/deepseek_coder_6.7b_instruct_2000/results.log"
OUT_PNG = HERE / "deepseek_three_curves.png"
OUT_PDF = HERE / "deepseek_three_curves.pdf"

# ── Extract accuracy from results.log ────────────────────────────────────────
RE_ACC = re.compile(r"iteration:\s*(\d+),\s*accuracy:\s*([\d.]+)")
RE_ERR = re.compile(r"iteration:\s*(\d+),\s*error:")

def extract_acc(log_path):
    """
    Parse results.log: successful lines have 'accuracy: X', error lines have 'error:'.
    Error iterations carry forward the previous round's accuracy.
    Leading errors (before the first success) are forward-filled with the first valid value.
    """
    raw = []
    last_acc = None
    with open(log_path) as f:
        for line in f:
            if m := RE_ACC.search(line):
                ...
                raw.append(last_acc)   # None if no success yet

    arr = np.array(raw, dtype=float)[:30]
...
n = len(acc)
x = np.arange(1, n + 1)
print(f"DeepSeek: {n} iterations, first={acc[0]:.4f}, last={acc[-1]:.4f}, best={acc.max():.4f}")
...
ax.set_title("DeepSeek-Coder-6.7B-Instruct")
