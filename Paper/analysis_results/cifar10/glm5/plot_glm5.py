"""
Plot three accuracy curves for GLM-5 (100 iterations):
  1. Best So Far  – running maximum
  2. Accuracy     – error iterations replaced by previous round's accuracy
                    (GLM-5 has no error iterations, so this equals the raw data)
...
RESULTS_LOG = HERE.parent.parent.parent / "experimental_result/cifar10/glm5_cifar10/results.log"
...
n = len(acc)
x = np.arange(1, n + 1)
print(f"GLM-5: {n} iterations, first={acc[0]:.4f}, last={acc[-1]:.4f}, best={acc.max():.4f}")
