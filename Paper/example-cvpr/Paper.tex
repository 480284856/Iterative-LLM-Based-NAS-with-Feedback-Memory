...
\begin{document}
	...
	\midrule
	OPRO~\cite{Yang2023OPRO} & Text prompts, discrete solutions & Top-K by quality (unbounded growth) & $\times$ & $\times$ Scores only & GPT-4, PaLM 2-L \\
	FunSearch~\cite{Romera2023FunSearch} & Math functions (Python) & Multi-island pools (unbounded) & $\times$ & $\times$ Population selection & Not specified \\
	EvoPrompting~\cite{Chen2023EvoPrompting} & Code-level NAS & Population diversity (implicit) & $\times$ & $\times$ Selection only & Requires fine-tuning \\
	ReEvo~\cite{Ye2024ReEvo} & Heuristic algorithms & Dual-layer reflection & $\times$ & $\checkmark$ Verbal comparison & Not specified \\
	\midrule
	\textbf{Ours} & \textbf{PyTorch NAS} & \textbf{K=5 sliding window} & \textbf{$\checkmark$ Yes} & \textbf{$\checkmark$ Diagnostic triples} & \textbf{4B/7B frozen} \\
	\bottomrule
	\end{tabular}
	\end{table*}
