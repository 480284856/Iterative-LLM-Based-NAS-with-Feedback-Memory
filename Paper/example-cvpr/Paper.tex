...
\begin{document}
	...
	\begin{itemize}
		...
		\item An empirical demonstration across two LLMs of different sizes (4B and 7B parameters) showing that the pipeline consistently improves architecture quality---from 38.0\% to 59.3\% (Qwen2.5-7B) and from 10.0\% to 37.9\% (Qwen3-4B)---with strong positive trends (Spearman $\rho \geq 0.67$, $p < 10^{-9}$).
	\end{itemize}
	...
	\paragraph{Language models.} To evaluate the pipeline across model sizes, we run separate experiments with two instruction-tuned LLMs from the Qwen family: \textbf{Qwen2.5-7B-Instruct} and \textbf{Qwen3-4B-Instruct}. Both models use the same generation parameters: temperature $0.7$, nucleus sampling $p = 0.9$, maximum 2,048 new tokens. A fixed seed with incremental call counter ensures reproducible but diverse generation across iterations. Each model is run for 100 iterations.
	...
	Figure~\ref{fig:accuracy} shows the accuracy trajectories for both models. Qwen2.5-7B (left) starts at a higher baseline (${\sim}38\%$) and steadily climbs to the $55$--$59\%$ range. Qwen3-4B (right) starts near random (${\sim}10\%$) and exhibits a pronounced rapid-improvement phase before plateauing around $35$--$38\%$. Both curves display characteristic convergence behavior with diminishing returns in later evaluations.
	
	\begin{table}[t]
		\caption{Comparison of the iterative NAS pipeline on CIFAR-10 (one-epoch proxy accuracy) across two LLMs. Both models show significant improvement over single-shot generation.} \label{table:summary}
		\centering
		\fontsize{8}{9.5}\selectfont
		\begin{tabular}{l r r}
			\toprule
			\textbf{Metric} & \textbf{Qwen2.5-7B} & \textbf{Qwen3-4B} \\
			\midrule
			Total iterations & 100 & 100 \\
			Successful evaluations & 77 & 41 \\
			Success rate & 77\% & 41\% \\
			\midrule
			Single-shot accuracy & $0.380$ & $0.100$ \\
			Best accuracy (eval \#) & $0.593$ \;(\#68) & $0.379$ \;(\#31) \\
			\midrule
			First 5 evals (mean $\pm$ SD) & $0.440 \pm 0.062$ & $0.134 \pm 0.076$ \\
			Last 5 evals (mean $\pm$ SD) & $0.571 \pm 0.032$ & $0.371 \pm 0.016$ \\
			Improvement (last $-$ first 5) & $+0.131$ & $+0.237$ \\
			\midrule
			Spearman $\rho$ \;($p$-value) & $0.670$ \;($< 10^{-10}$) & $0.787$ \;($< 10^{-9}$) \\
			Slope (per 10 evals) & $+0.012$ & $+0.065$ \\
			$R^2$ & $0.203$ & $0.557$ \\
			\bottomrule
		\end{tabular}
	\end{table}
	...
	\begin{figure}[t]
		...
		\caption{One-epoch proxy accuracy on CIFAR-10 across successful architecture evaluations for Qwen2.5-7B-Instruct (left, 77 evaluations) and Qwen3-4B-Instruct (right, 41 evaluations). Both models exhibit clear upward trends (Spearman $\rho = 0.67$ and $\rho = 0.79$, respectively).}
		\label{fig:accuracy}
	\end{figure}
	...
	The dominant validation failure mode for both models is tensor shape mismatches between layers, followed by \texttt{AttributeError}s from hallucinated layer names. Despite the different success rates, both models produce architectures that exhibit consistent improvement across their respective successful evaluations, confirming the pipeline's resilience to high failure rates.
	...
	Both models exhibit statistically significant positive correlations between evaluation index and accuracy (Qwen2.5-7B: $\rho = 0.67$; Qwen3-4B: $\rho = 0.79$), confirming that historical feedback memory enables systematic architectural improvement across different LLM sizes. Access to the last $K{=}5$ improvement attempts provides the LLM with sufficient context to identify recurring failure patterns and avoid previously unsuccessful strategies. Both accuracy curves display characteristic increasing behavior---rapid early gains followed by gradual increasing---mirroring patterns in traditional optimization and suggesting that the LLM-driven search exhibits meaningful optimization dynamics rather than random exploration.
	
	\paragraph{Effect of model size.}
	The comparison between the 7B and 4B models reveals a clear trade-off. The larger Qwen2.5-7B starts from a substantially higher baseline (38.0\% vs.\ 10.0\%), reaches a higher peak accuracy (59.3\% vs.\ 37.9\%), and generates structurally valid code nearly twice as reliably (77\% vs.\ 41\% success rate). However, the smaller Qwen3-4B exhibits a stronger relative improvement trend ($\rho = 0.79$ vs.\ $\rho = 0.67$, slope $5.4\times$ steeper), reflecting greater room for improvement from its lower starting point. This suggests that while larger models provide better ``raw material,'' the iterative feedback mechanism is proportionally more impactful for weaker generators.
