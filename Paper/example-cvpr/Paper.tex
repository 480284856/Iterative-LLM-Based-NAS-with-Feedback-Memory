...
\begin{document}
	...
	\midrule
	OPRO~\cite{Yang2023OPRO} & Text prompts, discrete & N/A & Top-K by quality (unbounded) & $\times$ & $\times$ Scores only & GPT-4, PaLM 2-L \\
	FunSearch~\cite{Romera2023FunSearch} & Math functions & Open Python & Multi-island pools (unbounded) & $\times$ & $\times$ Population selection & Not specified \\
	EvoPrompting~\cite{Chen2023EvoPrompting} & Code-level NAS & Open code & Population diversity (implicit) & $\times$ & $\times$ Selection only & Requires tuning \\
	ReEvo~\cite{Ye2024ReEvo} & Heuristic algorithms & Open code & Dual-layer reflection & $\times$ & $\checkmark$ Verbal comparison & Not specified \\
	LLMO~\cite{Zhong2024LLMO} & NAS (CIFAR) & \textbf{Closed cell-based} & $\times$ No memory & $\times$ & N/A (no code errors) & Gemini (large) \\
	\midrule
	\textbf{Ours} & \textbf{PyTorch NAS} & \textbf{Open code} & \textbf{K=5 sliding window} & \textbf{$\checkmark$ Yes} & \textbf{$\checkmark$ Diagnostic triples} & \textbf{4B/7B frozen} \\
	\bottomrule
	\end{tabular}
	\end{table*}
