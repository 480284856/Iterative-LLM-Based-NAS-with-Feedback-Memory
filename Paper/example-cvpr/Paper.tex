...
\begin{document}
	...
	NAS target: method aims at neural architecture search; Open code space search: search over executable code rather than fixed cell encodings; Failure model: structured modeling of code execution failures (e.g., diagnostic triples); Small model: demonstrated effective with $\leq$7B LLMs.
	Our method uniquely combines all four dimensions.}
	\label{table:comparison}
	\centering
	\fontsize{7.5}{9}\selectfont
	\begin{tabular}{l c c c c}
	\toprule
	\textbf{Method} & \textbf{NAS target} & \textbf{Open code space search} & \textbf{Failure model} & \textbf{Small model} \\
	\midrule
	OPRO~\cite{Yang2023OPRO} & $\times$ & $\times$ & $\times$ & $\times$ \\
	FunSearch~\cite{Romera2023FunSearch} & $\times$ & $\checkmark$ & $\times$ & $\times$ \\
	EvoPrompting~\cite{Chen2023EvoPrompting} & $\checkmark$ & $\checkmark$ & $\times$ & $\checkmark$ \\
	ReEvo~\cite{Ye2024ReEvo} & $\times$ & $\checkmark$ & $\checkmark$ & $\times$ \\
	LLMO~\cite{Zhong2024LLMO} & $\checkmark$ & $\times$ & $\times$ & $\times$ \\
	\midrule
